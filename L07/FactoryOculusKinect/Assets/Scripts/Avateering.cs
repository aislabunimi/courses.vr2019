...
namespace KinectV2Components
{
	...
	public class Avateering : KinectService
	{
		...
		/// Note: new Quaternion(0, -1, 0, -1) = Quaternion.Euler(0, -90, 0)
		/// </summary>
		private Dictionary<JointType, Quaternion> adjustRotations = new Dictionary<JointType, Quaternion>()
		{
		...
		public override void Elaborate(Body[] bodies)
		{
			...
			while (jointEnumerator.MoveNext())
			{
				...
				if (KinectAvatarController.IsLeaf(currentJoint.ToString()))
				{
				...
				GameObject rotatingJoint = controller.JointObjects[currentJoint.ToString()];
				...
				try
				{
					adjust = adjustRotations[currentJoint];
				}
		...
		public void EstimateJointOrientation(JointType currentJoint, Dictionary<JointType, Windows.Kinect.Joint> positions)
		{
			...
			GameObject jointObject = controller.JointObjects[currentJoint.ToString()];
